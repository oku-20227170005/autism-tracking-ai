...
import { createDailyLog, getDailyLogs } from "../services/api";

export default function DailyLogScreen({ user, onBack }) {
  ...
  async function handleSave() {
    ...
    try {
      const { response, data } = await createDailyLog(
        user.user_id,
        mood,
        sleepNumber,
        socialNumber,
      ...
      if (response.ok) {
        Alert.alert("Başarılı", "Günlük kayıt eklendi.");
        setMood("sakin");
        setSleepHours("8");
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <Text style={styles.title}>Günlük Kayıt</Text>
        ...
        <Text style={styles.sectionTitle}>Geçmiş Günlük Kayıtlar</Text>
        ...
        {logs.length === 0 ? (
          ...
          logs.map((log) => (
            <View key={log.id} style={styles.logCard}>
              ...
              <Text style={styles.logText}>Sosyal Etkileşim: {log.social_interaction}/10</Text>
